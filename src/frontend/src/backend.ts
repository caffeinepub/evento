...
export class Backend implements backendInterface {
    ...
    async addEvent(arg0: string, arg1: string, arg2: string): Promise<bigint> {
        if (this.processError) {
            try {
    ...
    async getEventsByCurrentUser(): Promise<Array<Event>> {
        if (this.processError) {
            try {
                const result = await this.actor.getEventsByCurrentUser();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
